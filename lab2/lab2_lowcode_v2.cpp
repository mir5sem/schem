...
int main() {
	...
	R1 = R1 + dp_ADDR; // 0005 ADD R1, dp_ADDR
	R1 = memory[R1_ADDR]; // 0006 LOAD R1, R1_ADDR
	R1 = R1 + prev_ADDR; // 0007 ADD R1, prev_ADDR
	...
	R1 += input_ADDR; // 0022 ADD R1, input_ADDR
	R3 = memory[R1]; // 0023 REG_MEM R3, R1
	R2 = memory[R1_ADDR]; // 0024 LOAD R2, R1_ADDR
	R2 += input_ADDR; // 0025 ADD R2, input_ADDR
	...
	if(R3 <= R2) // 0062 CMP R3, R2
		goto jmp3; // 0063 JLE to xxxxxxxxxxxxx
	...
	if(R1 >= R2) // 0090 CMP R1, R2
		goto reverse_loop_end; // 0091 JGE to xxxxxxxx
